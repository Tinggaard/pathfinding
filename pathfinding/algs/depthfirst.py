        ...
        for near in current.nearby[::-1]:
            ...
            if near is not None:
                ...
                if not visited[node.location]:
                    ...
                    self.mz[node.location] = self.CURRENT
                    self.mz[current.location] = [255, 255, 0]
                    plt.imshow(self.mz)
                    self.cam.snap()
                    self.mz[node.location] = self.EXPLORED
                    self.mz[current.location] = self.EXPLORED
